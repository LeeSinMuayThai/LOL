...
| **9W** | **El mejor del mundo**: ranking vivo de los mejores del momento (Top 5 a la derecha siempre, Top 20 al cierre de temporada). Se entra y se sale por mérito, rota mucho, de cualquier edad; distinto de los rivales de generación. Cero RNG (determinista por `hashCadena`) | 🚧 **9Wa hecho** (`core/topMundial.js` + `systems/topMundial.js` + `mundo.escenaAnual`; cero stream verificado byte-idéntico; r(nivel,rank)=0,96). Falta 9Wb (ganchos) / 9Wc (pantalla) / 9Wd (calibrar: jugador entra 12% de carreras con éxito, la vara §9W.6 es 50%). Ver §9W |
